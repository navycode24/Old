    ...
    buttons = [
            [
                InlineKeyboardButton(
                    text="sʜᴜғғʟᴇ ᴘʟᴀʏ", callback_data=f"psps{type} {id}"
                ),                                                   
            ],   
            [
                InlineKeyboardButton(
                    text="ᴘʟᴀʏ", callback_data=f"psp{type} {id}"
                ),
                InlineKeyboardButton(
...
async def spotify_play(_, message: Message):
    ...
    if url == "":
        await message.reply_photo(
                ...
                caption=(
                    "⭐️ **Give me a Link Or Use Browse Button Below**\n\n**Usage:**\n /spotify [Spotify Track Or Playlist Or Album Or Artist Link]\n\n➤ **Playing limit is 50 songs for playlists and albums** "
                ),
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="ᴍᴏʀᴇ", callback_data="cat pg1"),InlineKeyboardButton(text="Close", callback_data="close_btn"),]]))
    else:
        if url:
